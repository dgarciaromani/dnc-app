...
from src.data.database_utils import get_respondents, get_raw_data_forms, fetch_plan, get_linkedin_courses
from src.utils.dashboard_utils import create_donut_chart, get_dashboard_data
...
try:
    ...
    with col1:
        ...
        st.metric("👥 Total Encuestados", total_respondents, border=True)

    with col2:
        total_needs = len(get_raw_data_forms())
        st.metric("📋 Necesidades Levantadas", total_needs, border=True)

    with col3:
        total_activities = len(fetch_plan())
        st.metric("🎯 Actividades Formativas", total_activities, border=True)

    with col4:
        linkedin_courses = len(get_linkedin_courses())
        st.metric("🎓 Cursos LinkedIn", linkedin_courses, border=True)

    # Respondents Analysis
    with st.expander("👥 Análisis de Encuestados"):
    ...
    # Needs Analysis
    with st.expander("📋 Análisis de Necesidades"):
    ...
    with st.expander("🎯 Análisis de Actividades Formativas"):
        col1, col2 = st.columns(2)
